        ...
        stage('Switch Traffic (Nginx)') {
            steps {
                script {
                    ...
                    // Modify the nginx config in Jenkins workspace (sed -i works here, file is not volume-mounted)
                    sh "sed -i 's/${CURRENT_ENV}/${TARGET_ENV}/g' nginx/default.conf"
                    // Copy the modified config into the nginx container (docker cp handles volume mounts correctly, no rename issue)
                    sh "docker cp nginx/default.conf nginx:/etc/nginx/conf.d/default.conf"
